...
栈中存放的是栈帧（每个方法都会生成一个栈帧），栈帧由四个部分组成：局部变量临时表、操作数栈、动态链接和方法出口

这里对JVM的构成进行重新学习，JVM可以分为三部分：类加载系统、内存结构和执行引擎。其中内存结构也就是上述五部分。
类加载系统：类加载器，类加载过程
内存结构：上述五个加理解
执行引擎：解释器（逐行解释字节码文件，编译为机器指令并执行）、JIT即时编译器（将热点编译为机器码存储）、垃圾回收器
...
## JVM中的常量池
### 静态常量池和运行时常量池
**静态常量池**也称为Class常量池，每个.java文件都会编译为一个.class的字节码文件，每个.class文件都有一个常量池，因为其是由.java文件编译而来的，是不能更改的，所以叫做静态常量池。
...
```
[![pEqg9u4.png](https://s21.ax1x.com/2025/05/06/pEqg9u4.png)](https://imgse.com/i/pEqg9u4)
