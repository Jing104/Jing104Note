...
看到log(m+n)很自然想到二分查找，并且这个题目的数组是已经排序的，所以二分查找的思路不变，只是需要考虑边界条件。
...
```
# 寻找旋转排序数组中的最小值
力扣题目连接：[寻找旋转排序数组中的最小值](https://leetcode.cn/problems/find-minimum-in-rotated-sorted-array/description/?envType=study-plan-v2&envId=top-100-liked)
...
class Solution {
    public int findMin(int[] nums) {
        ...
        int left = 0;
        int right = n - 1;
        // 初始化 ans 为第一个数或最大整数，用来暂存最小值
        int ans = nums[0]; 
        
        while(left <= right){
            int mid = left + (right - left) / 2;
            if(nums[mid] <= nums[right]){
                // 说明 mid 到 right 是升序的，最小值在左半部分（包括 mid）
                ans = Math.min(ans, nums[mid]);
                right = mid - 1; // 强行收缩，避免死循环
            } else {
                // 说明左半部分是升序，且 nums[mid] 很大，最小值在右半部分
                left = mid + 1;
            }
        }
        return ans;
