...
实际训练过程中，模型也可能是一个线性的，输出数据可能千奇百怪，这个时候我们需要设置一个阈值，比如0.5，让小于0.5置为0，大于0.5置为1.此时我们需要引入sigmoid函数或者称为logistic函数：$$\frac{1}{1+e^{-x}}$$.$$z=\vec{w}\cdot\vec{x}+b$$$$g(z)=\frac{1}{1+e^{-z}}$$
